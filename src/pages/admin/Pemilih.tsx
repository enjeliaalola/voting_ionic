...
import Modal from './ModalPemilih';

import './Admin.css';

interface PemilihProps { }
const Pemilih: React.FC<PemilihProps> = () => {
	...
	const hapusData = (id) => {
		try {
			...
			axios.post('https://evotingosis.musky.site/hapusData.php', {id:id,tMaster:'pemilih'}, config)
				.then(response => {
					let a = response.data;
	...
	const fetchData = async () => {
		try {
			setShowLoading(true);
			const response = await axios.get('https://evotingosis.musky.site/getDataPemilih.php');
			let a = response.data;
			setData(a);
	...
	const openModal = (id,nama,nisn,kelas,username,password,gambar) => {
		setShowModal(true);
		setShowLoading(true);
		setModalData({
		  id: id,
	...
	const altText = "Pemilih Calon Ketua Osis atas nama ";
  return (
    <IonPage className = "overflow-auto">
		...
		<IonContent>
			<div className="calon mt-3">
				<h5 className="judulteks"><FontAwesomeIcon icon={faUser} /> Daftar Pemilih</h5>
				<IonList>
					{data.map((item) => (
						<div className="mb-2" key={item.id}>
						  <img alt={altText+item.nama} style={{height:"250px",width:"100%"}} src={item.gambar} />
						  <IonCardHeader>
							<IonCardTitle>{item.nama}</IonCardTitle>
							<IonCardSubtitle>NISN : {item.nisn}</IonCardSubtitle>
							<IonCardSubtitle>Kelas : {item.kelas}</IonCardSubtitle>
							<IonCardSubtitle>Username : {item.username}</IonCardSubtitle>
							<IonCardSubtitle>Password : {item.password}</IonCardSubtitle>
						  </IonCardHeader>
						  <div className="d-flex gap-2 mt-3 p-1 justify-content-between align-items-center aksicalon">
							...
							<button className="btn btn-primary" onClick={() => openModal(item.id,item.nama,item.nisn,item.kelas,item.username,item.password,item.gambar)}>
		...
		<ion-footer>
		  <div className="d-flex justify-content-between align-items-center hasil">
			<button className="btn btn-success" onClick={() => openModal('','','','','','','')}>
			  <FontAwesomeIcon icon={faPlusCircle} /> Tambah Daftar Pemilih
			</button>
		  </div>
		</ion-footer>
