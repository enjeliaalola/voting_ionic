...
import { faArrowLeft,faUserCircle,faUser,faUsers,faCheckCircle,faChartPie,faSignOut,faHome,faTimesCircle,faPencil } from '@fortawesome/free-solid-svg-icons';
...
const Beranda: React.FC<BerandaProps> = () => {
  ...
  return (
    <IonPage className = "overflow-auto">
		...
		<IonHeader>
			<IonToolbar>
			  <div className="d-flex justify-content-between align-items-center headerpage">
				<div><FontAwesomeIcon icon={faHome} /> &nbsp; Aplikasi eVoting</div>
				<div className = "keluar" onClick={keluarAplikasi}><FontAwesomeIcon icon={faSignOut} /></div>
			  </div>
			</IonToolbar>
		</IonHeader>
		<IonContent>
			<div className="mt-3 d-flex justify-content-between align-items-center ringkasan">
				<div className="alert alert-danger" role="alert">
					<h3><FontAwesomeIcon icon={faUser} /></h3>
					<h5>{localStorage.getItem('total_calon')} Calon</h5>
				</div>
				<div className="alert alert-primary" role="alert">
					<h3><FontAwesomeIcon icon={faUsers} /></h3>
					<h5>{localStorage.getItem('total_pemilih')} Pemilih</h5>
				</div>
			</div>
			<div className="d-flex justify-content-between align-items-center ringkasan">
				<div className="alert alert-success" role="alert" style = {{width:'100%'}}>
					<h3><FontAwesomeIcon icon={faCheckCircle} /></h3>
					<h5>{localStorage.getItem('sudah_memilih')} dari {localStorage.getItem('total_pemilih')} Pemilih sudah Melakukan Pemilihan</h5>
				</div>
			</div>
			<div className="ringkasanhasil">
				<h5 className="judulteks"><FontAwesomeIcon icon={faChartPie} /> Hasil Pelaksanaan eVoting</h5>
				<ChartPie />
			</div>
		...
		<ion-footer>
		  <div className="d-flex justify-content-between align-items-center bottommenu">
			<button className="btn btn-success" onClick={lihatBeranda}>
			  <FontAwesomeIcon icon={faHome} /> Beranda
			</button>
			<button className="btn btn-success" onClick={lihatCalon}>
			  <FontAwesomeIcon icon={faUser} /> Calon
			</button>
			<button className="btn btn-success" onClick={lihatPemilih}>
			  <FontAwesomeIcon icon={faUsers} /> Pemilih
			</button>
			<button className="btn btn-success" onClick={lihatHasil}>
			  <FontAwesomeIcon icon={faChartPie} /> Hasil
			</button>
		  </div>
		</ion-footer>
